...
- **Forbidden decisions:** Create a Shipwright ledger, design campaign, or adaptive multi-agent fan-out for the trivial change; take the reduction path before the gate; treat an unreadable platform reference as a reason to skip the gate or continue; create or modify project-level configuration (for example `package.json`) without an explicit user request or approval.
