...
- In the active session, record attributable current-session evidence for resolved Grok 4.5 family with effort rank `high` or stronger. Family-only harness labels such as `Cursor Grok 4.5` may prove family; compose them with status/model-picker High+ effort evidence when needed. A settings file, generic label, requested model, or unresolved display-only name is insufficient.
