...
Prove the test runner actually discovers tests in the chosen workspace before dispatching anyone — for example a `--listTests`-equivalent count compared against expectation, or a single known-good test run to completion. A workspace the project's tooling silently ignores produces green runs that verify nothing, and the failure is invisible in exit codes. Confirm too that the workspace can actually build and test at all: freshly created worktrees routinely lack installed dependencies and generated-but-gitignored files that no commit contains.
