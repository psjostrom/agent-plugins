...
The workspace must live somewhere the project's own tooling will actually operate on. Do not accept a workspace inside a tool-owned directory such as `.claude/`, `.cursor/`, or `.agents/` — these are conventionally excluded from test runners, typecheckers, and linters, so a workspace inside one is invisible to the very tools §11 depends on. If it was created there by default, relocate it before dispatching anyone. A fresh worktree is also the wrong workspace when the project needs generated, gitignored files to build — env config, version stamps, native artifacts — because a new worktree does not have them and §12 may require running the real app. Check for this before creating one. When it applies, work on a branch in the main checkout instead, and record the reason in the ledger. Do not treat the worktree step as satisfied by a workspace that cannot build.
