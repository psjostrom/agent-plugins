...
Accept an active model only when current runtime evidence resolves it to a concrete Opus model ID carrying a version, then compare that version numerically against the `4.6` floor. A higher major version meets the floor regardless of its minor version, so `claude-opus-4-6`, `claude-opus-4-7`, and `claude-opus-5` all pass. A context-size suffix such as `[1m]` describes the context window, not capability, and does not affect the comparison.
...
3. Validate attributable model-family evidence against the route floor. Accept absent effort for every route in this schema shape — including Sonnet and Opus — because effort cannot be requested or observed; a nonempty unknown effort label remains unverified.
